...
public static class GeneratorDriverRunResultExtensions
{
    ...
    )
    {
        ...
        if (assertOnErrors && result.Diagnostics.Length > 0)
            throw new XunitException(result.Diagnostics.GetExceptionMessage());

        result.Result.InternalShouldProduce(filePathEndsWith, expectedSource, assertOnErrors, message => throw new XunitException(message));
    }
    ...
    )
    {
        ...
        var generatedSource = result.Result.InternalGetSource(filePathEndsWith, assertOnErrors, message => throw new XunitException(message));
