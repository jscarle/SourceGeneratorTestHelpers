...
public static class GeneratorDriverRunResultExtensions
{
    ...
    /// <param name="throwOnErrors"><see langword="true" /> to throw on reported errors by the source generator, <see langword="false" /> othwerwise.</param>
    /// <param name="throwAction">The action to perform when an assertion fails. This typically involves throwing an exception with a descriptive error message.</param>
    /// <exception cref="ArgumentNullException">If <paramref name="result" /> is null.</exception>
    internal static void ShouldProduce(this GeneratorDriverRunResult result, string filePathEndsWith, string expectedSource, bool throwOnErrors, Action<string> throwAction)
    {
        ...
        if (throwOnErrors)
        {
            ...
            if (errors.Count > 0)
            {
                ...
                throwAction(errorBuilder.ToString());
            }
        }
        ...
        if (hasDifferences)
            throwAction($"There was a difference in the expected output generated by the source generator.\n\n{differences}");
    }
}
